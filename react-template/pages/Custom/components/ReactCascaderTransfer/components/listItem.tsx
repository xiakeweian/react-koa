...
const ListItem = (props: ListItemProps) => {
  ...
  const  indeterminate = (rowData: DataProps) => {
    ...
    if (rowData.label === '全选') {
      ...
      const aa = flatData.filter((item) => item.level === rowData.level && item.parent_value === rowData.prevParent && item.value !== rowData.value )

     const bb = flatTree(aa).filter((item) => item.label !=='全选')
     const isIndeterminate =  !bb.every((item) => item.checked === true) && !bb.every((item) => item.checked === false)
      ...
      >
        <span className="rct-flex">
          ...
          <span className="rct-label">
            {rowData.label === '全选'
              ? formatMessage({ id: 'create.camaping.checkAll' })
              : rowData.label}
          </span>
        </span>
        {rowData.children && rowData.children.length ? (
          <RightOutlined className="rct-pointer rct-left" style={{ fontSize: 12 }} />
        ) : null}
      </span>
    );
  };
  ...
  const listItemWithData = (
    <div>
      ...
      <div
        ...
        className={
          itemDataSource && itemDataSource.length <= 0 ? 'rtc-empty-content' : 'rtc-content'
        }
      >
        {(itemDataSource &&
